            ...
            foreach (var curSubObject in lstSubObjects.OrderBy(x => x.folderLevel))
            {
                ...
                while (i < gridFields.RowCount)
                {
                    ...
                    if (!GenUtil.IsNull(fieldName))
                    {
                        ...
                        {
                            bgWorkerSave.ReportProgress(0, string.Format("Error updating file, {0}, invalid field name or value, {1}={2}: {3}", filePath.Substring(filePath.LastIndexOf('/') + 0), fieldName, fieldVal, msg));
                            error_found = true;
                        }
                    }
                ...
                if (!error_found)
                {
                    bgWorkerSave.ReportProgress(0, string.Format("File, {0}, Updated Successfully.", filePath.Substring(filePath.LastIndexOf('/') + 0)));
                }
